...
const (
	trustedDealerImportPlanWireType             = "cggmp21.secp256k1.trusted-dealer-import-plan"
	trustedDealerImportPlanWireVersion   uint16 = 1
	trustedDealerContributionWireType           = "cggmp21.secp256k1.trusted-dealer-contribution"
	trustedDealerContributionWireVersion uint16 = 1
	trustedDealerImportPlanDigestLabel          = "cggmp21-secp256k1-trusted-dealer-import-plan-v1"
)
...
type trustedDealerCommitment struct {
	...
	ConstantCommitment []byte      `wire:"2,bytes,max_bytes=point"`
...
type trustedDealerImportPlanState struct {
	...
	PaillierBits   int                       `wire:"7,u32"`
	SecurityParams SecurityParams            `wire:"8,record"`
}

// TrustedDealerImportPlan is the canonical public import intent.
...
type trustedDealerContributionLifecycle struct {
	mu     sync.Mutex
	status uint8
}
...
type TrustedDealerContribution struct {
	...
	lifecycle *trustedDealerContributionLifecycle
}
...
func NewTrustedDealerImport(secretKey *SecretKey, option TrustedDealerImportOption, reader io.Reader) (*TrustedDealerImportPlan, map[tss.PartyID]*TrustedDealerContribution, error) {
	...
	if paillierBits < int(securityParams.MinPaillierBits) || (limits.Paillier.MaxModulusBits > 0 && paillierBits > limits.Paillier.MaxModulusBits) {
		return nil, nil, invalidPlanConfig(0, errors.New("trusted-dealer Paillier key size is outside allowed bounds"))
	}
	if reader == nil {
		reader = rand.Reader
	}
	...
	targetScalar, err := secpScalarFromSecret(secretKey.state.scalar)
	if err != nil {
		clear(targetChainCode)
		return nil, nil, err
	}
	scalars, err := splitCGGMPDealerScalar(reader, targetScalar, len(parties))
	if err != nil {
		clear(targetChainCode)
		return nil, nil, err
	}
	defer clearSecpPolynomial(scalars)
	chainCodes, err := splitCGGMPDealerChainCode(reader, targetChainCode, parties)
	if err != nil {
		clear(targetChainCode)
		return nil, nil, err
	}
	...
	for i, party := range parties {
		constant, err := secp.PointBytes(secp.ScalarBaseMult(scalars[i]))
		if err != nil {
			clear(targetChainCode)
			return nil, nil, err
		}
	...
	for i, party := range parties {
		value, err := secpSecretScalarFromScalar(scalars[i])
		if err != nil {
		...
		contributions[party] = &TrustedDealerContribution{
			...
			lifecycle: new(trustedDealerContributionLifecycle),
		}
	}
...
func GenerateTrustedDealerKeyShares(secretKey *SecretKey, option TrustedDealerImportOption, reader io.Reader) (*TrustedDealerImportPlan, map[tss.PartyID]*KeyShare, error) {
	...
	for _, party := range plan.state.Parties {
		guard, err := security.Guard(party, plan.state.Parties, tss.ProtocolCGGMP21Secp256k1, plan.state.SessionID, CGGMP21Policies())
		if err != nil {
			return nil, nil, err
		}
	...
	return cloneCGGMPTrustedDealerPlan(plan), shares, nil
}

func splitCGGMPDealerScalar(reader io.Reader, target secp.Scalar, count int) (shamir.Polynomial, error) {
	if count <= 0 {
		return nil, errors.New("empty trusted-dealer party set")
	}
	for {
		out := make(shamir.Polynomial, count)
		sum := secp.ScalarZero()
		for i := 0; i < count-1; i++ {
			value, err := secp.RandomScalar(reader)
			if err != nil {
				clearSecpPolynomial(out)
				return nil, err
			}
			out[i] = value
			sum = secp.ScalarAdd(sum, value)
		}
		out[count-1] = secp.ScalarSub(target, sum)
		if !out[count-1].IsZero() {
			return out, nil
		}
		clearSecpPolynomial(out)
	}
...
func splitCGGMPDealerChainCode(reader io.Reader, target []byte, parties tss.PartySet) (map[tss.PartyID][]byte, error) {
...
func destroyCGGMPContributions(contributions map[tss.PartyID]*TrustedDealerContribution) {
...
func (p *TrustedDealerImportPlan) Snapshot() (TrustedDealerImportPlanSnapshot, bool) {
	...
	for _, commitment := range p.state.Commitments {
		commitments[commitment.Party] = bytes.Clone(commitment.ConstantCommitment)
	}
...
func (p *TrustedDealerImportPlan) Digest() ([]byte, error) {
	...
	for _, commitment := range p.state.Commitments {
		...
		t.AppendBytes("constant_commitment", commitment.ConstantCommitment)
...
func (p *TrustedDealerImportPlan) ValidateWithLimits(limits Limits) error {
	...
	parties, err := validatePlanParties(p.state.Parties, p.state.Threshold, limits)
	if err != nil {
		return err
	}
	...
	if p.state.PaillierBits < int(p.state.SecurityParams.MinPaillierBits) || (limits.Paillier.MaxModulusBits > 0 && p.state.PaillierBits > limits.Paillier.MaxModulusBits) {
		return errors.New("trusted-dealer Paillier key size is outside allowed bounds")
	}
	...
	for i, commitment := range p.state.Commitments {
		...
		point, err := secp.PointFromBytes(commitment.ConstantCommitment)
		if err != nil {
	...
	sum, err := secp.PointBytes(secp.AddPoints(points...))
	if err != nil || !bytes.Equal(sum, p.state.PublicKey) {
...
func (p *TrustedDealerImportPlan) commitmentFor(party tss.PartyID) (trustedDealerCommitment, bool) {
	...
	for _, commitment := range p.state.Commitments {
		if commitment.Party == party {
			return trustedDealerCommitment{Party: party, ConstantCommitment: bytes.Clone(commitment.ConstantCommitment), ChainCodeCommit: bytes.Clone(commitment.ChainCodeCommit)}, true
		}
	}
	return trustedDealerCommitment{}, false
}
...
func cloneCGGMPTrustedDealerPlan(p *TrustedDealerImportPlan) *TrustedDealerImportPlan {
	if p == nil || p.state == nil {
		return nil
	}
	commitments := make([]trustedDealerCommitment, len(p.state.Commitments))
	for i, commitment := range p.state.Commitments {
		commitments[i] = trustedDealerCommitment{Party: commitment.Party, ConstantCommitment: bytes.Clone(commitment.ConstantCommitment), ChainCodeCommit: bytes.Clone(commitment.ChainCodeCommit)}
	}
	return &TrustedDealerImportPlan{state: &trustedDealerImportPlanState{
		SessionID: p.state.SessionID, Threshold: p.state.Threshold, Parties: p.state.Parties.Clone(),
		PublicKey: bytes.Clone(p.state.PublicKey), ChainCode: bytes.Clone(p.state.ChainCode),
		Commitments: commitments, PaillierBits: p.state.PaillierBits, SecurityParams: p.state.SecurityParams,
	}, limits: p.limits}
}
...
func (c *TrustedDealerContribution) validateForPlanLocked(plan *TrustedDealerImportPlan, party tss.PartyID) error {
	...
	if !bytes.Equal(c.state.PlanHash, planHash) || len(c.state.ChainCode) != bip32util.ChainCodeSize {
		return errors.New("trusted-dealer contribution binding mismatch")
	}
	scalar, err := secpScalarFromSecret(c.state.Scalar)
	if err != nil {
		return err
	}
	...
	encoded, err := secp.PointBytes(secp.ScalarBaseMult(scalar))
	if err != nil || !bytes.Equal(encoded, commitment.ConstantCommitment) {
		return errors.New("trusted-dealer contribution scalar does not match plan commitment")
	}
	wantChainCommit := bip32util.ChainCodeCommitment(cggmpChainCodeCommitLabel, c.state.SessionID, party, c.state.ChainCode)
...
// Format writes a redacted representation.
...
func (p *TrustedDealerImportPlan) UnmarshalBinaryWithLimits(in []byte, limits Limits) error {
	...
	if err := wire.Unmarshal(in, &state, wire.WithFrameLimits(limits.frameLimits(limits.State.MaxSerializedTrustedDealerPlanBytes)), wire.WithFieldLimits(limits.fieldLimits())); err != nil {
		return err
	}
...
func (c *TrustedDealerContribution) MarshalBinaryWithLimits(limits Limits) ([]byte, error) {
	...
	c.lifecycle.mu.Lock()
	defer c.lifecycle.mu.Unlock()
	if c.lifecycle.status != 0 || !c.state.SessionID.Valid() || c.state.Party == 0 || len(c.state.PlanHash) != sha256.Size || len(c.state.ChainCode) != bip32util.ChainCodeSize || validateSecretScalarStrict(c.state.Scalar) != nil {
		return nil, errors.New("invalid, claimed, or consumed trusted-dealer contribution")
	}
...
func (c *TrustedDealerContribution) UnmarshalBinaryWithLimits(in []byte, limits Limits) error {
	...
	if err := wire.Unmarshal(in, &state, wire.WithFrameLimits(limits.frameLimits(limits.State.MaxSerializedTrustedDealerContributionBytes)), wire.WithFieldLimits(limits.fieldLimits())); err != nil {
		if state.Scalar != nil {
	...
	if !state.SessionID.Valid() || state.Party == 0 || len(state.PlanHash) != sha256.Size || len(state.ChainCode) != bip32util.ChainCodeSize || validateSecretScalarStrict(state.Scalar) != nil {
		if state.Scalar != nil {
			state.Scalar.Destroy()
		}
		clear(state.ChainCode)
	...
	c.Destroy()
	*c = TrustedDealerContribution{state: &state, lifecycle: new(trustedDealerContributionLifecycle)}
	return nil
}
