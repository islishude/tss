...
func slowCryptoPresign(t *testing.T, shares map[tss.PartyID]*KeyShare, signers tss.PartySet) map[tss.PartyID]*Presign {
	...
	for _, party := range signers {
		p, err := loadPersistedPresignForTest(sessions[party])
		if err != nil {
			t.Fatalf("party %d persisted presign: %v", party, err)
		}
		presigns[party] = p
	}
	return presigns
...
func TestSlowCrypto_Keygen3of5Production(t *testing.T) {
	...
	pk := mustKeySharePublicKey(t, shares[1])
	for i := 2; i <= 5; i++ {
		if !bytes.Equal(pk, mustKeySharePublicKey(t, shares[tss.PartyID(i)])) {
...
func TestSlowCrypto_Refresh2of3Production(t *testing.T) {
	...
	for _, party := range parties {
		if !bytes.Equal(mustKeySharePublicKey(t, shares[party]), mustKeySharePublicKey(t, refreshed[party])) {
