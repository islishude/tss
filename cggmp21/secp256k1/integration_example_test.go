...
func Example_full_lifecycle() {
	...
	shares, err := runExampleCGGMPKeygen(parties, 2, cggmp.KeygenOptions{})
	if err != nil {
		panic(err)
	}
...
func ExampleStartReshare() {
	...
	shares, err := runExampleCGGMPKeygen(oldParties, 2, cggmp.KeygenOptions{})
	if err != nil {
		panic(err)
	}
	oldPublicKey := shares[1].PublicKeyBytes()
...
func ExampleDeriveNonHardenedBIP32() {
	parties := []tss.PartyID{1, 2}
	shares, err := runExampleCGGMPKeygen(parties, 2, cggmp.KeygenOptions{EnableHD: true})
	if err != nil {
		panic(err)
	}
	path := []uint32{0, 1}
