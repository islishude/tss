...
const (
	// ChainCodeSize is the fixed length of chain code
	ChainCodeSize = 32
)
...
func AggregateChainCode(parties tss.PartySet, chainCodes map[tss.PartyID][]byte) ([]byte, error) {
	...
	for _, id := range parties {
		if len(chainCodes[id]) != ChainCodeSize {
			return nil, fmt.Errorf("party %d chain code is %d bytes, want %d", id, len(chainCodes[id]), ChainCodeSize)
		}
		for i := range out {
