...
func TestMarshalRoundTrip(t *testing.T) {
	...
	version, fields, err := UnmarshalFields(raw, "test.type")
	if err != nil {
		t.Fatal(err)
	}
	if version != 1 {
	...
	raw2, err := MarshalFields(1, "test.type", fields)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnmarshalRejectsWrongTypeID(t *testing.T) {
	...
	if _, _, err := UnmarshalFields(raw, "other.type"); err == nil {
		t.Fatal("wrong type id accepted")
	}
}

func FuzzWireUnmarshalFields(f *testing.F) {
	raw, err := MarshalFields(1, "test.type", []Field{{Tag: 1, Value: []byte("seed")}})
	if err != nil {
		f.Fatal(err)
	}
	f.Add(raw)
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		version, fields, err := UnmarshalFields(data, "test.type")
		if err != nil {
			return
		}
		again, err := MarshalFields(version, "test.type", fields)
		if err != nil {
			t.Fatal(err)
		}
