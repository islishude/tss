...
func TestMTAProductShares(t *testing.T) {
	...
	if err := VerifyStart(params, startDomain, start.Message, aCommit, skA.PublicKey, rpB, startProof); err != nil {
		t.Fatal(err)
	}
	if err := VerifyStart(params, []byte("other-start"), start.Message, aCommit, skA.PublicKey, rpB, startProof); err == nil {
		t.Fatal("start proof verified under wrong domain")
	}
	if err := VerifyStart(params, startDomain, start.Message, aCommit, skA.PublicKey, rpA, startProof); err == nil {
		t.Fatal("start proof verified for wrong verifier aux")
	}
	response, betaShare, err := Respond(params, nil, startDomain, responseDomain, start.Message, startProof, aCommit, bSecret, bCommit, skA.PublicKey, skB.PublicKey, rpB, rpA)
	if err != nil {
		t.Fatal(err)
	}
	if _, _, err := Respond(params, nil, startDomain, responseDomain, start.Message, nil, aCommit, bSecret, bCommit, skA.PublicKey, skB.PublicKey, rpB, rpA); err == nil {
