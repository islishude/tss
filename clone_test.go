...
func (s testShare) Clone() testShare {
	out := testShare{
		id: s.id,
	}
...
func TestCloneSliceNilInput(t *testing.T) {
	got := CloneSlice[testShare](nil)
...
func TestCloneSliceEmptyInput(t *testing.T) {
	got := CloneSlice([]testShare(nil))

	if got != nil {
		t.Fatalf("CloneSlice(empty) = %#v, want nil", got)
	}
}

func TestCloneSlicePreservesElements(t *testing.T) {
	in := []testShare{
	...
	if !reflect.DeepEqual(got, in) {
		t.Fatalf("CloneSlice() = %#v, want %#v", got, in)
	}
}
...
func TestCloneSliceDeepCopiesElements(t *testing.T) {
	...
	if in[0].data[0] != 1 {
		t.Fatalf("mutating cloned element mutated original: got original data[0] = %d, want 1", in[0].data[0])
	}
}

func TestCloneSliceDoesNotReuseBackingStorage(t *testing.T) {
	in := []testShare{
		{id: "alice", data: []byte{1, 2, 3}},
	}
	...
	if len(got) != len(in) {
		t.Fatalf("len(CloneSlice()) = %d, want %d", len(got), len(in))
	}
