...
- `github.com/islishude/tss/gg20/secp256k1`: experimental GG20-style secp256k1 threshold ECDSA keygen, presign, online signing, and evidence verification.
- `internal/wire`: strict TLV encoding used by binary envelope, key-share, and presign records.
- `internal/curve`, `internal/shamir`, `internal/paillier`, `internal/mta`, and `internal/zk`: protocol-local cryptographic helpers.
...
GG20 key shares include Paillier private material and proof data needed by the signing path. Old shares without Paillier/ZK fields can be decoded as records, but `StartPresign` rejects them and requires rerunning keygen.
...
GG20 secp256k1 separates offline presign from online signing. Presign records contain local one-use `k_i` and `sigma_i` values and must not be shared. `StartSignDigest` marks a presign consumed before producing any outbound online signing message.
